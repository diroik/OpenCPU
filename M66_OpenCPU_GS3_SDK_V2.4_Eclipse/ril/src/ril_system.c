...
{
	//Ql_Debug_Trace("<-- SecureData_Read_Callback len=%d -->\r\n", len);
...
s32 Ql_SecureData_Read(u8 index, u8* pBuffer, u32 len)
{
    ...
    //Ql_Debug_Trace("<-- Ql_SecureData_Read start Ql_MEM_Alloc size=%d -->\r\n",ptrCus.readSize);
    ...
    //Ql_Debug_Trace("<-- Ql_SecureData_Read command=[%s] -->\r\n", ATstr);
    Ql_memset(ptrCus.pHexBuf, 0x00, ptrCus.readSize+1);

    //Ql_Debug_Trace("<-- Ql_SecureData_Read Ql_RIL_SendATCmd=[%s] -->\r\n", ATstr);
    ret = Ql_RIL_SendATCmd(ATstr, Ql_strlen(ATstr), SecureData_Read_Callback, &ptrCus, 0);
    //Ql_Debug_Trace("<-- Ql_SecureData_Read ret=%d IsDataReadSuccess=%d -->\r\n", ret, ptrCus.IsDataReadSuccess);
    if(ret <0)
        return ret;
