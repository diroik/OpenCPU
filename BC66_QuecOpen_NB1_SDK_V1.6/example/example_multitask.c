 ...
 * Description:
 * ------------
 *   This example demonstrates how to use multitask function with APIs in QuecOpen.
...
void MutextTest(int iTaskId)  //Multiple tasks Run this function at the same time
{

    APP_DEBUG("<---I am Task %d--->\r\n", iTaskId);
    Ql_OS_TakeMutex(s_iMutexId,0xFFFFFFFF); 
    APP_DEBUG("<---I am Task %d,Ql_OS_TakeMutex---->\r\n", iTaskId);   //Another Caller prints this sentence after 3 seconds
    Ql_Delay_ms(3000);                                                             
...
void SemaphoreTest(int iTaskId)
{
    ...
    while(1)
    {
		//iRet = Ql_OS_TakeSemaphore(s_iSemaphoreId, 0xFFFFFFFF);//TRUE or FLASE indicate the task return immediately or should wait infinitely.

		iRet = Ql_OS_TakeSemaphore(s_iSemaphoreId, 0);//non blocking.
        if(TRUE == iRet)
        {
            APP_DEBUG("\r\n<-----I am Task[%d]: take semaphore OK!-->", iTaskId);
			
		    Ql_OS_TakeMutex(s_iSemMutex,0xFFFFFFFF);
            s_iTestSemNum--;
            Ql_OS_GiveMutex(s_iSemMutex);
			APP_DEBUG("\r\n<-----You can also apply for semaphores[%d]!---->", s_iTestSemNum);
            break;
        }
        else
        {
    ...
    Ql_Delay_ms(3000);     //Release Semaphore after 3 seconds;  
