 ...
 * Description:
 * ------------
 *   This example demonstrates how to use LwM2M function with APIs in QuecOpen.
 *
 * Usage:
 * ------
 *   Compile & Run:
...
void proc_main_task(s32 taskId)
{
    ...
    APP_DEBUG("<--QuecOpen: LwM2M Client.-->\r\n");
		...
		{     
			switch (msg.param1)
            {
    			...
    			case URC_LWM2M_REG:
				{
					...
					{
						APP_DEBUG("<---urc_lwm2m_register failed, error = %d -->\r\n",msg.param2);
					}
					break;
				}
				...
				case URC_LWM2M_NOTIFY:
				{
					lwm2m_urc_param_ptr = msg.param2;
					APP_DEBUG("<-- +QLWNOTIFY: %d -->\r\n", lwm2m_urc_param_ptr->error_code);
    ...
    if (LwM2M_TIMER_ID == timerId)
    {
        ...
        {        
            case STATE_NW_QUERY_STATE:
            {
                ...
                {
                    m_lwm2m_state = STATE_NW_GET_LOCALIP;
                }
                break;
            }
			...
			case STATE_LwM2M_REG:
            {
                ...
                if (RIL_AT_SUCCESS == ret)
                {
                    APP_DEBUG("<--send a register request to the IoT platform.\r\n");
                    m_lwm2m_state = STATE_LwM2M_TOTAL_NUM;
                }else
                {
                    APP_DEBUG("<--send a register request to the IoT platform failure,error=%d.-->\r\n",ret);
                }
                break;
            }
			...
			case STATE_LwM2M_NOTIFY:
            {
				...
				if (RIL_AT_SUCCESS == ret)
                {
                    APP_DEBUG("<-- Notify the Data to Server\r\n");
                    m_lwm2m_state = STATE_LwM2M_TOTAL_NUM;
