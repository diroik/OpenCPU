 ...
 * Description:
 * ------------
 *   This example demonstrates how to use MQTT function with APIs in QuecOpen.
 *
 * Usage:
 * ------
 *   Compile & Run:
...
static Enum_Mqtt_State m_mqtt_state = STATE_NW_QUERY_STATE;
...
void proc_main_task(s32 taskId)
{
    ...
    APP_DEBUG("<--QuecOpen: MQTT Client.-->\r\n");
		...
		{     
			switch (msg.param1)
            {
                ...
                case URC_MQTT_SUB:
				{
					...
					if(0 == mqtt_urc_param_ptr->result)
					{
						if(128 != mqtt_urc_param_ptr->sub_value[0])
						{
						...
						{
							APP_DEBUG("<-- Subscribe Topics Rejected by Server -->\r\n");
						}
					}
					else if(1 == mqtt_urc_param_ptr->result)
					{
						APP_DEBUG("<-- Data packet retransmission, retry times = %d -->\r\n", mqtt_urc_param_ptr->sub_value[0]);
					}
					else if(2 == mqtt_urc_param_ptr->result)
					{
						APP_DEBUG("<-- Data packet send failed -->\r\n");
					}
				}
    			break;
				case URC_MQTT_PUB:
    ...
    if (MQTT_TIMER_ID == timerId)
    {
        ...
        {        
            case STATE_NW_QUERY_STATE:
            {
                ...
                {
                    m_mqtt_state = STATE_NW_GET_LOCALIP;
                }
                break;
            }
