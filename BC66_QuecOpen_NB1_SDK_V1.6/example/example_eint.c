 ...
 * Description:
 * ------------
 *   This example demonstrates how to use eint function with APIs in QuecOpen.
 *
 * Usage:
 * ------
 *   Compile & Run:
...
void proc_main_task(s32 taskId)
{
    ...
    **************************************************************/
	...
	if(ret != 0)
    {
        APP_DEBUG("<--QuecOpen: Ql_EINT_RegisterFast fail.-->\r\n");
        return;
    }
    APP_DEBUG("<--QuecOpen: Ql_EINT_RegisterFast OK.-->\r\n");
    #else
    //Registers an EINT I/O, and specify the interrupt handler.
    ret = Ql_EINT_Register(pinname,callback_eint_handle, NULL);
	if(ret != 0)
    {
        APP_DEBUG("<--QuecOpen: Ql_EINT_Register fail.-->\r\n");
        return;
    }
    APP_DEBUG("<--QuecOpen: Ql_EINT_Register OK.-->\r\n");
    #endif

    ret = Ql_EINT_Init(pinname, EINT_EDGE_RISING, 50, 0, 0);
    if(ret != 0)
    {
        APP_DEBUG("<--QuecOpen: Ql_EINT_Init fail.-->\r\n"); 
        return;    
    }
    APP_DEBUG("<--QuecOpen: Ql_EINT_Init OK.-->\r\n"); 
