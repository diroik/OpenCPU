 ...
 * ------------
 *		
 *	This example demonstrates how to use LwIP API to connect UDP server, 
 ...
 *   Compile & Run:
 *
 *     Set "GLOBAL_EXPORT_FLAG += __EXAMPLE_LWIP_UDPCLIENT__" in makefile file. And compile the 
 ...
 * 
 ****************************************************************************/
#ifdef __EXAMPLE_LWIP_UDPCLIENT__
...
static void lwip_udpclient_demo_entry(ST_MSG msg);
void proc_main_task(void)
{
	...
	APP_DEBUG("<-- QuecOpen: lwip UDPclient Example -->\r\n");
	Ql_SleepDisable();
    ...
    while (1)
    {
        ...
        {		
        	case MSG_ID_URC_INDICATION:
			{
	            switch (msg.param1)
	            {
	            	...
	            	case URC_EGPRS_NW_STATE_IND:
					{
		                ...
		                {
							ceregCheckTimes = 1;							
							SendMsgToApp(UDP_STATE_SOC_CREATE);
			...
			{
				lwip_udpclient_demo_entry(msg);
			}
				break;
			case MSG_ID_APP_RECV_DATA:
			{
    			addr_length = sizeof(servaddr);
				ret = recvfrom(m_socketid, m_recv_buf, RECV_BUFFER_LEN,MSG_DONTWAIT,(struct sockaddr *)&servaddr,&addr_length);
				if(0 < ret)
				{
					APP_DEBUG("<-- recv data:(");
...
{	
	switch(msg.param1)
	{
		...
		{	
			...
			if (m_socketid >= 0)
			{
				SendMsgToApp(UDP_STATE_SOC_SEND);
		...
		case UDP_STATE_SOC_SEND:
		{
			...
			sendto( m_socketid, m_pCurrentPos, strlen(m_pCurrentPos), 0, (struct sockaddr *)&servaddr, sizeof(servaddr));
            APP_DEBUG("<--socket send success-->\r\n");	
			break;
		}
		case UDP_STATE_SOC_CLOSE:
		{
			if (m_socketid > 0)
            {
               close(m_socketid);
			...
			SendMsgToApp(UDP_STATE_TOTAL_NUM);
